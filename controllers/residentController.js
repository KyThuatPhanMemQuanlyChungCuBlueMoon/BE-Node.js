  ...
  try {
    ...
    if (idCard) {
      ...
      if (residentExists) {
        return res
          .status(400)
          .json({ message: "Resident with this ID card already exists" });
      }
    }
...
exports.updateResident = async (req, res) => {
  try {
    ...
    if (
      req.body.household &&
      req.body.household !== resident.household?.toString()
    ) {
...
exports.deleteResident = async (req, res) => {
  try {
    ...
    if (resident.household) {
      ...
      if (
        household &&
        household.householdHead?.toString() === resident._id.toString()
      ) {
